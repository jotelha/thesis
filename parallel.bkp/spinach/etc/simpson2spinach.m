...
% information into Spinach data structures.
%
% ilya.kuprov@oerc.ox.ac.uk

function [sys,inter]=simpson2spinach(filename)

% Read the file
fid=fopen(filename);
simpson_file=textscan(fid,'%s','delimiter','\n');
simpson_file=simpson_file{:};
fclose(fid);
...
if exist('spinsys_start','var')&&exist('spinsys_end','var')
    disp(['simpson2spinach: SIMPSON spinsys{} section located between lines ' num2str(spinsys_start) ' and ' num2str(spinsys_end) '.']);
else
    error('simpson2spinach: could not locate SIMPSON spinsys{} section');
end
...
for n=spinsys_start:spinsys_end
    ...
    if (length(current_line)>=5)&&strcmp(current_line(1:5),'shift')
        ...
        if nnz(current_line(6:end)=='p')~=2
            error('simpson2spinach: only ppm specification (# #p #p # # # #) is supported for Zeeman tensors.');
        end
    ...
    elseif (length(current_line)>=10)&&strcmp(current_line(1:10),'quadrupole')
        ...
        if nnz(current_line(11:end)=='p')~=0
            error('simpson2spinach: only Hz specification (# # # # # # #) is supported for quadrupole tensors.');
        end
