...
function fid=jlh_range_pulse_acquire(nodes,ppn,spin_system,parameters,L,rho)
...
%jlh -------------------------------------------------
            orientations_per_node = floor(grid_size / nodes);
            remaining_orientations = mod(grid_size,nodes);
            ...
            report(spin_system, sprintf('jlh_pulse_acquire: %d orientation / %d nodes = %d orientations per node; %d remaining orientations.',grid_size,nodes,orientations_per_node,remaining_orientations));
            ...
            %tmp = sprintf( strcat('jlh_', right_now, '_%d\n'), 1:nodes);
            job_name = cell(nodes,1);
            job_identifier = cell(nodes,1);
            jobfile_name = cell(nodes,1);
            logfile_name = cell(nodes,1);
            output_file_name = cell(nodes,1);
            %output_file_name = zeros(nodes,256);
            lower_boundary = ((1:nodes)-1)*orientations_per_node+1;
            upper_boundary = (1:nodes)*(orientations_per_node);
            ...
            for i=1:nodes
                ...
                fprintf(fileID, '#PBS -l walltime=00:10:00 \n');
                ...
                report(spin_system, ['jlh_pulce_acquire: Creating jobfile "' jobfile_name{i} '": ' jobfile_content ]);
                ...
                report(spin_system, strcat('jlh_pulce_acquire: Calling "', qsub_call, '"...'));
            ...
            last_set_starting_point = nodes*orientations_per_node+1;
            ...
            while ~allStopped
                ...
                report(spin_system, sprintf('jlh_pulse_acquire: %d Jobs stopped. Waiting 10s...', stopped));
                pause(10);
            end
            pause off;
    end
